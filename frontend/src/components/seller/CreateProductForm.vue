    ...
    <section class="bg-white rounded shadow border border-gray-200 p-6">
      ...
      <form @submit.prevent="onSubmit" class="space-y-4">
        ...
        <div class="flex gap-4 mt-4">
          ...
          <button type="button" @click="cancel" class="px-6 py-2 rounded bg-rose-300 text-white">Cancel</button>
        </div>
      </form>
    </section>
  </div>
...
async function onSubmit() {
  try {
    ...
    // Try to create product via Supabase directly for immediate visibility
    ...
    if (insertErr) {
      ...
      console.warn('Supabase insert failed, falling back to API:', insertErr)
      await createProduct({
        title: form.title,
        description: form.description,
        price: form.price,
